...
## 1. Project Overview
This laboratory work focuses on the implementation of the **Recursive Newton-Euler Algorithm (RNEA)** for a 7-DOF manipulator. The goal is to analyze the inverse dynamics of the robot under different kinematic scenarios (Dynamic, Quasi-static, and Static).

**Selected Robot:** Franka Emika Panda (7-DOF)

## 2. Manipulator Model and Parameters

### Denavit-Hartenberg (DH) Parameters
The robot model is loaded using the Modified DH convention. 
- **Type:** 7 Revolute joints.
...
*   **Masses ($m$):** Identified link masses ranging from 4.97kg to 0.73kg.
...
*   **Drive Inertia ($J_m$):** $0.0002 \text{ kg}\cdot\text{m}^2$.
*   **Friction:** Viscous ($B=0.0015$) and Coulomb ($T_c=[0.4, -0.43]$).
*   **Gear Ratios ($G$):** $1:100$ for all joints.
*   **Constraints ($q_{lim}$):** Generalized coordinate limits set to $\pm 2.89$ rad.

## 3. Trajectory Planning
A smooth trajectory was planned between two arbitrary configurations:
*   **$q_{start}$:** `[0, 0, 0, 0, 0, 0, 0]` (Zero Pose)
*   **$q_{end}$:** `[pi/4, -pi/3, -pi/4, pi/3, -pi/3, pi/4, pi/4]`
...
Joint torque graphs were plotted for all 7 joints. These plots illustrate the contribution of various forces (Inertia, Friction, Gravity) over the course of the trajectory.
...
The static scenario ($\tau_{static}$) reveals that gravity is the primary source of joint torque. Joints supporting the largest mass segments (Joint 2 and 4) show significant static torques ($>30\text{ Nm}$).
...
### Final Summary
The experiment confirms that for the Franka Emika Panda, gravity compensation is the most critical component of the control law, though full dynamics are required for high-speed tracking accuracy.
